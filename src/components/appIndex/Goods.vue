<template>
  <div>
    <div class="all">
      <div  class="one" v-for="item in goods" :key="item.id" @click="goDetails(item.id)">
        <div>
...
export default {
  ...
  computed: {
    goods() {
      return this.$store.state.vuex_goods;
    },
  },
  mounted: function () {
    this.$store.dispatch("getGoods", "服装");
  },
  ...
  methods: {
    ...
    async goDetails(id){
      await this.$store.dispatch('getGoodsDetails',id)
